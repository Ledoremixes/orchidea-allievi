...
export default function AdminPanel() {
  ...
  const autoGeneratedMonthRef = useRef("");
  ...
  useEffect(() => {
    if (activeSection !== "payments" || loading) return;
    const rowsToGenerate = monthlyPaymentRows.filter((row) => row.status === "da_generare" && row.billable);
    if (rowsToGenerate.length === 0) return;

    const autoKey = `${paymentMonth}:${rowsToGenerate.map((row) => row.enrollment.id).sort().join(",")}`;
    if (autoGeneratedMonthRef.current === autoKey) return;
    autoGeneratedMonthRef.current = autoKey;
    ...
    async function autoGenerateMonthlyDues() {
      ...
      const payloads = rowsToGenerate.map((row) => {
        ...
        const amount = Number(row.monthlyPrice || 0) * period.months;
        return {
          ...
          importo: amount,
          periodo: buildPaymentPeriodLabel(monthStart, period.end, row.cycle),
          scadenza: monthStart,
          stato: "da_pagare",
          metodo: null,
          tipo_quota: "corso",
      ...
      if (result.ok && result.inserted > 0) {
        showSuccess(`Quote aggiornate automaticamente per ${monthHumanLabel(paymentMonth)}.`);
        await loadAdminData();
      }
    }

    autoGenerateMonthlyDues();
  }, [activeSection, loading, paymentMonth, monthlyPaymentRows]);
